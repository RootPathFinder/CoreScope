...
- **Configure companion radio** card (`#/repeaters`, requires `apiKey`): set the companion's LoRa parameters over serial without the MeshCore app. Pick a **region preset** (US/Canada recommended = 910.525 MHz, BW 62.5 kHz, SF7, CR5; US legacy = 910.525/250/SF11/CR5; EU/UK = 869.525/250/SF11/CR5) or **Custom**, optionally set TX power (dBm), and click **Apply to companion**. The poller applies `CMD_SET_RADIO_PARAMS` / `CMD_SET_RADIO_TX_POWER` (applied live, no reboot), then re-reads self-info and shows the values the device now reports back — so you can confirm the change stuck. **Warning:** wrong values disconnect the companion from your mesh and managed repeaters; match your region.
